...
We concentrate on the Zipf key distribution and  SSD hardware. We run two sets of experiments: in the first  all MSLAB chunks are allocated on-heap, and in the second --  off-heap. For the serialized segments we set parameter $A$ to $0.1$, for less frequent in-memory flushes. Figure~\ref{fig:write_only_off_heap} presents the throughput speedup using flat and serialized 
