...
\inpurple{Write-only experiments consist of 500M writes. Mixed workload experiments consist of a load phase (write-only) and a run phase with concurrent reads and writes. 
...
Finally, we study the scans behaviour on SSD hardware with zipf keys distribution. We chose to practice the standard YCSB scans workload ($workload$ $e$), where 95\% of the operations are short range queries and the other 5\% are put operations.  The workload is measured after loading 50GB of data (500M write operations) to make sure most if not all keys have some version of the data on disk. The latency speedup (or slowdown) is presented in Figure~\ref{fig:scans}.
}
\begin{figure}[htb]
