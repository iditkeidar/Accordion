...
\anastas{
\paragraph{Scans}
Finally, we study the scans behavior on SSD hardware with zipf keys distribution. We chose to practice the standard YCSB scans workload ($workload$ $e$), where short ranges of records are queried with small amount of puts in the background. Among 12 threads, 11 are proceeding with scanning 100 records and one thread writes. The workload is measured after 500M writes are loaded. 
}
