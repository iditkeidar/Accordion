We fully implemented \sys\ in HBase, and it is generally available as the production default in HBase 2.0 and up. 
...
We also experiment with the write-only workload on the serialized segments, where the index is allocated on the MSLAB chunk (Section~\ref{ssec:offheap}). We concentrate on the Zipf key distribution and the SSD hardware. Here memory is managed using MSLAB memory allocation. We run two sets of experiments: in the first set all MSLAB chunks are allocated on-heap and in the second set  all MSLAB chunks are allocated off-heap. Recall that in the serialized flavour of \basic\/ the serialized index itself is also allocated on a chunk . Figure~\ref{fig:write_only_off_heap} presents the throughput speedup of the flat and serialized flavours of \basic\/ policy over \none. For the flat \basic, the speedup is $27\%$ on-heap and $29\%$ off-heap. For the serialized \basic, the speedup is $33\%$ on-heap and $44\%$ off-heap. The gain from the serialized \basic\/ is larger for the off-heap case, because when index is taken off-heap even less work is left to JVM GC. 
...
Finally, we study the scans behaviour on SSD with Zipf keys distribution. We chose to exercise  the standard YCSB scans workload (workload $e$), where 95\% of $500\,000$ operations are short range queries (50 entries in expectation), and the other 5\% are put operations.  The workload is measured after loading 50GB of data (500M write operations). The latency slowdown of \basic\/ over \none\/ is presented in Figure~\ref{fig:scans}. Here we also experiment with different size of the pipeline (parameter $S$). With only two segments in the pipeline ($S=2$) the search for a key is a bit faster than with a longer pipeline ($S=5$). We can see that the slowdown for short pipeline is negligible -- up to $3\%$. When the pipeline may grow to five segments (as in the write-only experiments) the performance gap slightly increases to $2\%-8\%$. These results resemble the SSD reads slowdown.
}
\begin{figure}[htb]
...
The peak throughput is achieved with $S=5$ on SSD and $S=4$ on HDD. The affect of the parameter $S$ on the read performance was seen in the scans latency in Figure~\ref{fig:scans}. We learn from there that the gap in scans latency between $S=5$ and $S=1$ is up to $5\%$.
