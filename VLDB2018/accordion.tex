...
Section~\ref{ssec:offheap} discusses off-heap allocation of  the serialized memory segments.
...
\emph{segments} ordered by creation time. Each segment is an index over a collection of data cells.
...
%
Get and scan operations retrieve data from all  segments,  similarly to a traditional LSM store read from multiple files. 
...
The indexed data cells are unaffected by the index's flattening.
...
Prior to \sys, HBase allocated its MemStore on-heap, using a standard Java skiplist for the active buffer and the snapshot. \sys\ continues to use the same data structure -- skiplist for the active segment, but adopts arrays for the flat segments and snapshot. 
...
called \emph{pojo} -- Pure Old Java Object -- or an address in a bulk-allocated array called \emph{mslab} -- MemStore local allocation buffer~\cite{hbasemslab}. 
HBase mslabs may be allocated off-heap. 

\begin{figure*}[tbh]
\center
\includegraphics[width=0.9\columnwidth]{serializing.pdf} 
\caption{Serializinf process... TBD}
\label{fig:on-heap}
\end{figure*}
%\begin{figure}[tbh]
%\center
...
\sys's off-heap version takes this approach one step further, and allocates the index of the flat segments on mslab as well (on-heap or off-heap). Such a segment (denoted \emph{serialized segment}) has its index and data allocated on the same mslab, meaning they are either both off-heap or both on-heap. Serialized segment forgoes the intermediate cell
objects, and has the array entries point directly to the the mslabs holding keys and values, as illustrated in Figure~\ref{fig:on-heap}.  
