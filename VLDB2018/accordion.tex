...
called \emph{pojo} -- Pure Old Java Object -- or an address in a bulk-allocated array called \emph{mslab} -- MemStore local allocation buffer. 
...
\sys's off-heap version takes this approach one step further, and allocates the flat segments on off-heap as well. Those segments are allocated on mslab, which is in turn allocated from off-heap. It therefore forgoes the intermediate cell
