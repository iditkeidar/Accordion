...
\sys's off-heap version takes this approach one step further, and allocates the index of the flat segments on mslab as well (on-heap or off-heap). Such a segment (denoted \emph{serialized segment}) forgoes the intermediate cell
objects, and has the array entries (in the off-heap mslab) point directly to the the mslabs holding keys and values (off-heap or on-heap), as illustrated in Figure~\ref{fig:off-heap}.  
