...
\sys\ re-applies the classic LSM tree design principles to the memory store. Namely, it partitions this store
 ...
 data before it is written to disk. \sys's data organization is illustrated in Figure~\ref{fig:accordion};
...
improves the system's write throughput by up to $\inred{47\%}$, and reduces the tail read latency by up to 
...
overhead is more substantial: the improvements are highly correlated with  reduction in garbage collection (GC) time. 
