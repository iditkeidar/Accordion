...
in parallel from 12 client threads. We study the Zipf and the uniform key distributions. Depending on the setting 
(workloads, policies and hardware type), our experiments run for approximately 1.5 hours to over 12 hours.  
...
We repeated our experiments with large data values (1KB). In this context, \sys\/ produced more modest 
performance gains and I/O savings (\inred{$XX\%$}). This is not surprising because the algorithm primarily 
works on reducing the metadata handling overhead. 
