...
Measured on Zipf and Uniform key distributions, on SSD and HDD hardware. 
} 
...
hypothesis. We see a clear negative correlation between the GC cycles and the write throughput. 

\begin{figure}[htb]
\includegraphics[width=\figw]{Figs/gc-throughput-log2.png}
\caption{GC time (cumulative, per minute) vs write throughput, log-log scale, measured for multiple \sys\/ 
policies and configurations. The metrics are in clear negative linear correlation. 
}
