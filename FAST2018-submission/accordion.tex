...
\inred{It also allows for off-heap allocation, as discussed in Section~\ref{ssec:offheap} below.}
...
Each skiplist node holds two references to  \emph{cell} objects, one for the key and one for the value. 
The arrays are populated by cells, alternating between keys and values, the former being used for binary search. 
...
objects, and has the off-heap array entries in the flat segments point directly to the the off-heap mslabs. 
...
[[Idit: 1. What about the keys? 2. Add pictures.]]
}
