...
The \ingreen{in-memory} flush makes the active segment immutable and adds it to the pipeline; it creates a new active segment to replace it. \ingreen{Unless the new number of immutable segments in the pipeline exceeds $S$, the \emph{flattening of intra-segment search indices} starts immediately on the newly added segment. This implies, replacing dynamic segment indices such as skiplists by 
...
Flushes to disk work the same way as in a standard LSM tree. A disk flush \ingreen{shifts to snapshot all pipeline segments. The pipeline is emptied and ready to absorb new flat segments. The snapshot's list of immutable, flat segments is not a part of the pipeline, and is going to be freed once written to storage.} The background flush process merges the snapshot's 
